...
impl Random {
    ...
    pub fn untwist_verify(state0: &[u32; N], state1: &[u32; N]) {
        ...
        // (s0[623] & 0x80000000) >> 1 == (s1[396] ^ s1[623]) & 0x40000000
        // (s0[623] & 0x80000000 != 0) == ((s1[396] ^ s1[623]) & 0x40000000 != 0)
        assert_eq!(
            state0[N - 1] & 0x80000000 != 0,
            (state1[M - 1] ^ state1[N - 1]) & 0x40000000 != 0,
        );

        for i in N - M..N - 1 {
            // Solving for LSB of state0[i + 1]:
            //
            // s1[227] == s1[0] ^ ((s0[227] & 0x80000000) >> 1) ^ ((s0[228] & 0x7ffffffe) >> 1) ^ ((s0[228] & 0x1) * 0x9908b0df)
            // s1[227] & 0x80000000 == (s1[0] ^ ((s0[227] & 0x80000000) >> 1) ^ ((s0[228] & 0x7ffffffe) >> 1) ^ ((s0[228] & 0x1) * 0x9908b0df)) & 0x80000000
            // s1[227] & 0x80000000 == (s1[0] ^ ((s0[228] & 0x1) * 0x9908b0df)) & 0x80000000
            // s1[227] & 0x80000000 == (s1[0] ^ ((s0[228] & 0x1) << 31)) & 0x80000000
            // ((s0[228] & 0x1) << 31) & 0x80000000 == (s1[0] ^ s1[227]) & 0x80000000
            // ((s0[228] & 0x1) != 0) == ((s1[0] ^ s1[227]) & 0x80000000 != 0)
            assert_eq!(
                state0[i + 1] & 0x1 != 0,
                (state1[i - (N - M)] ^ state1[i]) & 0x80000000 != 0,
            );
        }
    }
